...
//!   2. Если нет — находим `ariy-helper.exe`, запускаем его с
...
const HELPER_FILENAME: &str = "ariy-helper.exe";
...
const HELPER_FILENAME_TRIPLET: &str = "ariy-helper-x86_64-pc-windows-msvc.exe";
...
pub async fn ensure_running() -> Result<()> {
    ...
    bail!(
        "helper-сервис установился, но не отозвался с актуальным протоколом за {}с. \
         Проверьте services.msc → AriyHelper",
        PING_TIMEOUT_AFTER_INSTALL.as_secs()
    )
}

/// Найти `ariy-helper.exe` в нескольких возможных локациях:
///   1. `<exe-dir>/ariy-helper.exe`            — dev (target/debug,
///                                                    target/release)
///                                                    или prod если Tauri
///                                                    стрипает triplet;
///   2. `<exe-dir>/ariy-helper-<triplet>.exe`  — prod если Tauri
///                                                    оставляет triplet
///                                                    после bundle;
