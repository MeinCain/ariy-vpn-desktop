...
//! но с суффиксом `ariy-helper` чтобы не путать с main-крашами.
//!
...
fn crashes_dir() -> Option<PathBuf> {
    ...
    if let Some(local) = std::env::var_os("LOCALAPPDATA") {
        return Some(PathBuf::from(local).join("AriyVPN").join("crashes"));
    }
    if let Some(program) = std::env::var_os("PROGRAMDATA") {
        return Some(PathBuf::from(program).join("AriyVPN").join("crashes"));
    }
    None
}
...
fn write_crash_dump(info: &panic::PanicHookInfo<'_>) -> std::io::Result<()> {
    ...
    let ts = SystemTime::now()
        ...
        .map(|d| d.as_secs())
        .unwrap_or(0);
    let path = dir.join(format!("{ts}-ariy-helper.txt"));
    ...
    writeln!(f, "Ariy VPN helper crash dump")?;
    writeln!(f, "----------------------")?;
    writeln!(f, "component: ariy-helper")?;
