...
//
//  1. Собирает `ariy-helper.exe` в release-режиме.
//  2. Копирует получившийся `target/release/ariy-helper.exe` в
//     `src-tauri/binaries/ariy-helper-<triplet>.exe` — Tauri ожидает
...
//
// Если `ariy-helper.exe` заблокирован запущенным сервисом —
...
// Если файла helper-а ещё нет, шаг 1 (cargo build --bin ariy-helper)
...
info("компилирую ariy-helper.exe в release...");
...
if (buildResult.status !== 0) {
  ...
  if (
    ...
    fail(
      "ariy-helper.exe в target/release/ заблокирован запущенным сервисом.\n" +
        "         Останови сервис админом перед сборкой:\n" +
        "           sc stop AriyHelper\n" +
        "           src-tauri\\target\\release\\ariy-helper.exe uninstall"
    );
  }
...
const sourceHelper = join(TARGET_RELEASE, "ariy-helper.exe");
...
try {
  ...
  info(
    `helper скопирован → binaries/ariy-helper-${TRIPLET}.exe (${size} МБ)`
  );
} catch (e) {
  fail(`не удалось скопировать helper: ${e.message}`);
}

// ── 2b. Дублируем под именем `ariy_helper.exe` для tauri-bundler ─
// Tauri 2 при сборке installer'а конвертирует имена `[[bin]]` из
// kebab-case (`ariy-helper`) в snake_case (`ariy_helper.exe`)
...
const sourceHelperUnderscored = join(TARGET_RELEASE, "ariy_helper.exe");
try {
  copyFileSync(sourceHelper, sourceHelperUnderscored);
  info(
    `helper дублирован → target/release/ariy_helper.exe (для tauri-bundler)`
  );
} catch (e) {
