...
//
// Обёртка над `cargo build --bin ariy-helper` для npm `predev` хука.
...
//      ВЫЗЫВАЕМ `sc stop AriyHelper` через PowerShell `-Verb RunAs`
...
function buildHelper() {
  return spawnSync(
    "cargo",
    ["build", "--manifest-path", "src-tauri/Cargo.toml", "--bin", "ariy-helper"],
    {
...
function stopHelperServiceElevated() {
  console.log(
    "[predev] пытаюсь остановить AriyHelper-сервис (потребуется UAC)…",
  );
  ...
  const psCommand = `
    try {
      $p = Start-Process sc.exe -ArgumentList 'stop','AriyHelper' \
...
if (isFileLocked(stderr)) {
  console.log(
    "\n[predev] ariy-helper.exe заблокирован запущенным сервисом.",
  );
