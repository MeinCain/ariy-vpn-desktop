 ...
 * `ariy:open-add-subscription` event'ом, который ловит этот компонент
...
export function SubscriptionMeta() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("ariy:open-add-subscription", onOpen);
    return () =>
      window.removeEventListener("ariy:open-add-subscription", onOpen);
  }, []);
