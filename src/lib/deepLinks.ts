 ...
 *
 *  Управление VPN:
 *   ariy://connect | open                подключить выбранный сервер
 *   ariy://disconnect | close            отключиться
 *   ariy://toggle                        переключить состояние
 *   ariy://status                        вынести окно вперёд
 *
 *  Импорт подписки (поддерживается оба синтаксиса):
 *   ariy://add?url=<encoded-url>         query-форма
 *   ariy://add/<encoded-url-or-base64>   path-форма
 *   ariy://import/<...>                  alias
 *   ariy://onadd/<url>                   импорт + сразу подключение
 *   ariy://import?data=<base64>          альтернативный query-параметр
 *
...
export function handleDeepLink(rawUrl: string) {
  ...
  if (parsed.protocol !== "ariy:") {
    console.warn("[deep-link] чужая схема:", parsed.protocol);
    return;
  }

  // host у ariy://action — пустой на одних платформах, заполнен на
  ...
  switch (action) {
    ...
    case "export": {
      // 12.D — `ariy://export` сохраняет backup в Documents и
    ...
    case "import-from-url": {
      // 12.D — `ariy://import-from-url/<url>` — скачать backup и
    ...
    case "import-settings": {
      // 12.D — `ariy://import-settings?data=<base64-or-json>` или
      // `ariy://import-settings/<base64>` — открыть preview-modal
    ...
    case "autorouting": {
      ...
      //   ariy://routing/{add|onadd}/{base64|url}
      //   ariy://autorouting/{add|onadd}/{url}
 ...
 * запуск (когда приложение запустили кликом по ariy://...).
 */
